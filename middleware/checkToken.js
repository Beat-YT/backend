...
module.exports = async (req, res, next) => {
    ...
    if (req.headers.authorization ? !req.headers.authorization.startsWith("bearer ") || !req.headers.authorization.substring(7) == undefined : true) return res.json({
    ...
    try {
        ...
        if (decoded.am == "client_credentials") {
            if (!clientTokens.includes(req.headers.authorization.split(" ")[1])) throw new Error("invalid")
        } else {
